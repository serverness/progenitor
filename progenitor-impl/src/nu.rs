...
impl Generator {
    /// Generate a `nu`-based CLI.
    pub fn nu(&mut self, spec: &OpenAPI, crate_name: &str) -> Result<TokenStream> {
        ...
        let methods = raw_methods
            .iter()
            .map(|method| self.nu_method(method))
            .collect::<Vec<_>>();

        let nu_ops = methods.iter().map(|op| &op.nu_fn);
        let execute_ops = methods.iter().map(|op| &op.execute_fn);
        let trait_ops = methods.iter().map(|op| &op.execute_trait);

        let nu_fns = raw_methods
            .iter()
            .map(|method| format_ident!("nu_{}", sanitize(&method.operation_id, Case::Snake)))
            .collect::<Vec<_>>();
        ...
        let code = quote! {
            ...
            /*#(
                ...
                impl Command for #nu_variants {
                    fn name(&self) -> &str {
                        stringify!(#nu_variants)
                    }
                    ...
                    fn description(&self) -> &str {
                        ...
                        fn run(
                            ...
                            #execute_fns(engine_state, stack, call, input)
                        }
                }
            )**/
    ...
    fn nu_method(&mut self, method: &crate::method::OperationMethod) -> CliOperation {
        let CliArg {
            parser: parser_args,
            consumer: consumer_args,
        } = self.nu_method_args(method);
        ...
        let fn_name = format_ident!("nu_{}", &method.operation_id);
    ...
    fn nu_method_args(&self, method: &crate::method::OperationMethod) -> CliArg {
        ...
        for param in &method.params {
            ...
            let consumer = quote! {
                if let Some(value) =
                    call.get_flag::<#arg_type_name>(engine_state, stack, #arg_name)?
                {
            ...
            match details {
                typify::TypeDetails::Struct(struct_info) => {
                    for prop_info in struct_info.properties_info() {
                        self.nu_method_body_arg(&mut args, prop_info)
                    }
                }
        ...
        let body_json_consumer = maybe_body_type_id.map(|body_type_id| {
            ...
            quote! {
                if let Some(value) =
                    call.get_flag::<std::path::PathBuf>(engine_state, stack, "json-body")?
                {
    ...
    fn nu_method_body_arg(&self, args: &mut CliOperationArgs, prop_info: TypeStructPropInfo<'_>) {
        ...
        if scalar && !args.has_arg(&prop_name) {
            ...
            let consumer = quote! {
                if let Some(value) =
                    call.get_flag::<#prop_type_ident>(
                        engine_state, stack, #prop_name,
                    )?
                {
